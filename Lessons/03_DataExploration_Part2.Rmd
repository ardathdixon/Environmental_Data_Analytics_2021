...
> Answer: Bar plot is great to show one categorical variable's count comparisons. Histograms are good to show a continuous variable's count comparisons. Frequency polygon graphs are good to compare several continuous variables' distributions to each other. Box and whisker plots are helpful to compare categorial variables' distributions. Scatterplots compare values' relationships between two continuous variables. 

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: I learned that there is a date gap around 1980 with no measurements collected (datetime & discharge mean scatter plot).


How can multiple options for data exploration inform our understanding of our data?

> Answer: Multiple options for data exploration help us to better know which tools can show what characteristics about a data set. They can guide our processes to more directly answer a question at hand.
...
> Answer: One pattern is that there are more mean measurements than max/min measurements (frequency polygon graph). This could be due to measurement capacities at the early stages, where the tool reported solely one measurement per day rather than three values.
