...
vector1[3] # gives the 3rd item from vector1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # creates a matrix w 5 rows and 4 columns, listing values 1-20 vertically first to fill out the box
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # also creates a matrix w 5 rows and 4 columns of values 1-20, but listing horizontally first
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #same as matrix2, but names the columns and rows with listed names

matrix1[4, ] # shows the fourth row of matrix1
matrix1[ , 3] # shows the third column of matrix1
matrix1[c(12, 14)] # shows the 12th and 14th items
matrix1[c(12:14)] # shows the items 12 to 14 (12th, 13th, and 14th items)
matrix1[2:4, 1:3] # includes values 2-4 in the first column, and includes columns 1 to 3
...
dataframe1 <- data.frame(d,e,f) # consolidates the vectors into a dataframe
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # renames the dataframe columns

dataframe1[1:2,] # shows rows 1 and 2
dataframe1[c("ID","Passed")] # shows only ID and Passed columns
dataframe1$ID
...
```{r}
Durham_high_F <- c(51,51,52,52,51,51,41,42,36,40)
Durham_low_F <- c(40,29,33,28,35,33,26,22,20,22)
```
...
Durham_high_C <- CelsiusToFahrenheit(Durham_high_F)
Durham_low_C <- CelsiusToFahrenheit(Durham_low_F)
...
```{r}
df_DurhamTemps <- data.frame(Durham_high_F,Durham_low_F,
                             Durham_high_C,Durham_low_C)
names(df_DurhamTemps) <- c("High Temp (F)","Low Temp (F)",
...
```{r}
summary(df_DurhamTemps) #can do summary statistics for the whole df
sd(df_DurhamTemps$`High Temp (F)`) #need to do std dev one column at a time
sd(df_DurhamTemps$`Low Temp (F)`)
sd(df_DurhamTemps$`High Temp (C)`)
sd(df_DurhamTemps$`Low Temp (C)`)
