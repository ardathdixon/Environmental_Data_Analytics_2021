...
> Answer: The output will show in the Console. The Console reports what is being installed step by step. For an umbrella package like tidyverse, it will show all the packages included in the umbrella package.
...
> ANSWER: The README file is in the folder Data > Metadata. Placing the data here makes it better organized if/when someone is examining multiple data sets. Using ./ enables those using other machines to follow the same path and fine the Metadata with the same script. The Metadata itself includes a link as to its source, so the user can follow it and download an updated version if applicable.
...
> Answer: They were imported as NA or blank cells. The fact that two commas were next to each other without a value in between them told R that the cell was empty.THe commas separating each value (.csv doc) indicated to R which values belong in which cells. If the column was characters, R left the empty cell blank. If the column was numbers, R inserted NA into the cell.
...
> Answer: If the missing observations are in the beginning or end of the dataset, then they can be smoothly removed without impacting analysis conclusions. However, for time-series analyses, removing NAs can impact the conclusions and skew the data (e.g. changes in water flow from daily measurements). NAs could be easily removed for data points that are not compared over time (e.g. geometric measurements of specific trees/animals/etc, qualitative data from survey respondants, etc.).
...
```{r savingdata}
# Note the added "." again for .Rmd #received error message until I deleted the 2nd .
