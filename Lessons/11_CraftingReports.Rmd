...
## error reading the csv... not sure why?
## says: Error in file(file, "rt") : cannot open the connection
## but then worked when ran the whole chunk as one!
...
```{r}
tn_time_display <- ggplot(Nutr_filtered, aes(x=sampledate, y=tn_ug, color= lakename))+
    geom_point(size = 3)+
    geom_smooth(size = .1, alpha = 0.1, se = FALSE)+ ## doesn't seem to change anything
